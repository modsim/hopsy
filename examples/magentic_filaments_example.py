...
class Potentials:
    ...
    def fene_potential(self, x):
        ...
        U_FENE = (-self.K_f * self.r_f**2) * np.log(1 - (r / self.r_f)**2) / 2 if (r/self.r_f)**2 < 1 else np.inf
    ...
    def wca_potential(self, x):
        ...
        for i in range(n_particles):
            ...
            U_WCA += 0 if r > self.r_cut else 4 * self.eps * ((self.sigma / r)**12 - (self.sigma / r)**6) - self.U_LJ_cut
...
def draw_state(x, sigma, ax, color, n_dof = 4):
    ...
    for i in range(n_particles - 1):
        particle_i_coord = x[4*i+2:4*i+4] 
        particle_j_coord = x[4*(i+1)+2:4*(i+1)+4] 
        ax.plot([particle_i_coord[0], particle_j_coord[0]], [particle_i_coord[1], particle_j_coord[1]], color=color)
...
def main():
    ...
    print(compute_sample_average(lambda x: np.linalg.norm(x[-4:-2]), np.array(run.data.states[-1])))
