...
class MGaussian:
    ...
    def compute_negative_log_likelihood(self, x):
        return -np.log(np.exp(-self.model1.compute_negative_log_likelihood(x)) +
...
if __name__ == "__main__":
    ...
    num_procs = 1
    mc = [hopsy.MarkovChain(proposal=hopsy.GaussianCoordinateHitAndRunProposal,
                            problem=problem,
                            parallelTemperingSyncRng=syncRng,
                            exchangeAttemptProbability=0.15,
                            starting_point=0.9 * np.ones(2)) for i in range(num_procs)]
    ...
    acc, samples = hopsy.sample(markov_chains=mc, rngs=rng, n_samples=550000, thinning=1, n_procs=num_procs)
