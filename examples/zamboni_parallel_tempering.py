...
if __name__ == "__main__":
    ...
    model = x3cfluxpy.ILESimulator('../../models/zamboni/unimodal-1,2-GLC/zamboni-unimodal-1,2-GLC.fml', sim_method='emu')
    ...
    starting_point = 0.9*model.last_params+0.1*hopsy.compute_chebyshev_center(problem).flatten()

    syncRng = hopsy.RandomNumberGenerator(42)
    num_procs = 1
    mc = [hopsy.MarkovChain(proposal=hopsy.DikinWalkProposal,
                            problem=problem,
    ...
    acc, samples = hopsy.sample(markov_chains=mc, rngs=rng, n_samples=10000, thinning=50, n_procs=num_procs)
