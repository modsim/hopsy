...
def create_shared_memory(state_shape, state_type=_s.numpy.float64):
    shared_memory_size = (_s.numpy.dtype(state_type).itemsize * _s.numpy.prod(state_shape))
...
class PyParallelTemperingChain:
    ...
    def draw(self, rng: _c.RandomNumberGenerator, thinning: int = 1):
        for i in range(thinning - 1):
            self._draw(rng)
        return self._draw(rng)

    def _draw(self, rng: _c.RandomNumberGenerator):
        return self._parallel_normal_draw(rng)
        # seed = rng()
        # _s.random.seed(seed)
        # if _s.random.uniform(a=0, b=1) < self.exchange_attempt_probability:
        #     return self._parallel_normal_draw(rng)
        # else:
        #     return self._parallel_tempering_exchange(rng)

    def _parallel_normal_draw(self, rng):
        return self.markov_chain.draw(rng=rng)

    def _parallel_tempering_exchange(self, rng):
        # TODO: find out who has to swap
        # barrier
    ...
    # TODO
    # def select_states_from_full_output(self, parallel_tempering_samples: np.ndarray, beta:float):
    #     index = self.temperature_ladder.index(beta)
    #     return parallel_tempering_samples[index,:,:]
...
def add_box_constraints(
    ...
    dimensions.This
    will
    extend: attr:`hopsy.Problem.A` and: attr:`hopsy.Problem.b`
    of
    the
    returned:
    ...
    should
    be
    constrained and which
    contains
    the
    matrix: math:`A` and vector: math:`b` in: math:`Ax \leq
    b
    `.

    :param
    lower_bound: Specifies
    the
    lower
    bound(s).
...
def add_equality_constraints(
    ...
    Adds
    equality
    constraints as specified.This
    will
    change: attr:`hopsy.Problem.A` and: attr:`hopsy.Problem.b`.
    The
    equality
    constraints
    are
...
def is_polytope_empty(
    ...
    The
    result is only as precise as the
    LP
    solver and settings
    used.
    We
...
def is_problem_polytope_empty(problem: _c.Problem):
    ...
    the
    problem
    with polytope given by Ax < b and optionally Sx=h has a solution x.
...
def compute_chebyshev_center(problem: _c.Problem, original_space: bool = False):
    ...
    that if A and b
    are
    transformed(e.g.rounded), the
    chebyshev
    center is computed in the
    transformed
    space.To
    trigger
    a
    ...
    hopsy.Problem
    problem: Problem
    for which the Chebyshev center should be computed and which contains the matrix:math: `A` and vector:math: `b` in:math: `Ax \leq
    b
    `.
    :param
    bool
    ...
    the
    chebyshev
    center is required in the
    original
    space, use
    original_space = True.Only
    works if the
...
def simplify(problem: _c.Problem):
    ...
    than
    thresh.
    Thresh is defined in the
    LP
    settings
    singleton and refers
    to
    `PolyRound < https: // pypi.org / project / PolyRound / > `_
    settings.
    Simplification is typically
    the
    first
    step
    before
    ...
    automatically
    when
    round is called, because
    it is required
    for efficient and effective rounding.

    Parameters
    ----------
    problem: hopsy.Problem
    for which the polytope should be simplified

    Returns
    -------
    hopsy.Problem
    Problem
    with simplified polytope.
...
def round(problem: _c.Problem):
    ...
    The
    unrounding
    transformation: math:`T` and shift: math:`s`
    will
    be
    stored in: attr:`hopsy.UniformProblem.transformation`
    and:attr: `hopsy.UniformProblem.shift`
    of
    the
    returned
    problem.Its
    left - hand
    side
    operator: attr:`hopsy.UniformProblem.A` and
    the
    right - hand
    side: attr:`hopsy.UniformProblem.b`
    of
    the
    polytope
    will
...
def sample(
    ...
    r"""
    sample(markov_chains, rngs, n_samples, thinning=1, n_procs=1)

    Draw
    ``n_samples``
    from every passed

    chain in ``markov_chains``
    using
    the
    respective
    random
    ...
    attributes or ``acceptance_rate``, which
    will
    then
    be
    recorded and returned.All
    attributes
    of:
    ...
    in_memory: bool
    Flag
    for enabling or disabling in -memory saving of states and metadata.
    callback: derived
    from hopsy.Callback
    ...
    attributes
    defined in ``record_meta``.
    If
    the
    attribute
    was
    not found(e.g.because
    of
    a
    typo), it
    will
...
def ess(*args, **kwargs):
    r"""
    ess(data, series=0, method="bulk", relative=False, prob=None, dask_kwargs=None)
    ...
    MCMC
    samples
    with ``data.shape == (n_chains, n_draws, dim)``.
    series: int
    Compute
    a
    series
    ...
    be
    computed
    for ``data[:, :n] for n in range(series, n_draws+1, series)``.
    For
    the
    default
    value
    ...
    probability
    value
    for "tail", "quantile" or "local" ess functions.
    n_procs: int = 1
    In
    combination
    with "series": compute
    ...
    kwargs
    passed
    to: func:`~arviz.wrap_xarray_ufunc`.
...
def mcse(*args, **kwargs):
    r"""
    mcse(data, series=0, method="mean", prob=None, dask_kwargs=None)
...
def rhat(*args, **kwargs):
    r"""
    rhat(data, series=0, method="rank", dask_kwargs=None)
    ...
    diagnostic
    tests
    for lack of convergence by comparing the variance
    between
    multiple
    chains
    to
    ...
    achieved,
    the
    between - chain and within - chain
    variances
    should
    be
    identical.To
    ...
    where: math:`W` is the
    within - chain
    variance and: math:`\hat
    {V}
    ` is the
    posterior
    variance
    estimate
    for the pooled rank-traces.This is the potential scale reduction factor, which
    converges
    to
    unity
    when
    ...
    with `scipy.stats.rankdata`.
        Each
    chain is split in two and normalized
    with the z-transform following Vehtari et al.(2019).
...
def _svd_rounding(samples, polytope):
    ...
    based
    on
    samples, as suggested in https: // drops.dagstuhl.de / opus / volltexte / 2021 / 13820 / pdf / LIPIcs - SoCG - 2021 - 21.
    pdf
    This is used in the
    multipase_sampling
    function
...
def run_multiphase_sampling(
    ...
    runs
    multiphase
    sampling as suggested in https: // drops.dagstuhl.de / opus / volltexte / 2021 / 13820 / pdf / LIPIcs - SoCG - 2021 - 21.
    pdf
    limit_singular_value_ratio = 2.3
    ...
    works
    well
    for uniform sampling, BUT it is not officially supported for non-uniform targets!
    """
