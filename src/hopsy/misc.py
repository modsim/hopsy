...
class PyParallelTemperingChainGroup(_s.MutableSequence):
    ...
    def insert(self, index, item):
        """Insert was implemented to make this class a MutableSequence"""
...
def MarkovChain(
    ...
    if (
        proposal is None
        and isinstance(problem.model, _c.Gaussian)
        and problem.transformation is None
        and problem.shift is None
    ):
...
def compute_chebyshev_center_and_radius(
    ...
    """
    Computes the Chebyshev center and radius of the (non-unique) largest inscribed ball in the polytope defined by :math:`Ax < b`.
    ...
    :param hopsy.Problem problem: Problem for which the Chebyshev center should be computed and which contains the matrix :math:`A` and vector :math:`b` in :math:`Ax < b`.
...
def compute_chebyshev_center(problem: _c.Problem, original_space: bool = False):
    """
    Computes the Chebyshev center, that is the midpoint of a (non-unique) largest inscribed ball in the polytope defined by :math:`Ax < b`.
    Note that if A and b are transformed (e.g. rounded), the chebyshev center is computed in the transformed space. To trigger a backtransform, use the parameter `original_space=True`.
    However, the chebyshev center is NOT affine invariant.
...
def round(problem: _c.Problem, simplify=True):
    ...
    :param hopsy.Problem problem: Problem that should be rounded and which contains the matrix :math:`A` and vector :math:`b` in :math:`Ax < b`.
