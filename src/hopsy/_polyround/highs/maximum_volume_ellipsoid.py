...
def iterative_solve(
    ...
    verbose_print(settings, "highs", "maximum volume ellipsoid found")
...
def run_mve(A, b, x0, reg, verbose=False):
    ...
    return x, np.linalg.cholesky(get_NSPD(E2)), msg, delta_b, delta_s
...
def check_convergence(
    ...
    verbose_print(verbose, "highs", "inner MVE iteration=" + str(iter))
    if (
        ...
        verbose_print(verbose, "highs", "stopped making progress; restarting")
        x = x + x0.squeeze()
        msg = 2
    ...
    if (res < tol * (1 + bnrm) and rmu <= minmu) or (
        ...
        verbose_print(verbose, "highs", "inner MVE converged")
        x = x + x0.squeeze()
        msg = 1
